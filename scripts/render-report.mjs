...
function renderBusinessMap(map) {
  const altitudes = (map.altitudes ?? []).map((altitude) => {
    ...
    const nodeHtml = nodes.map((node) => {
      return `<article class="map-node"><h3>${esc(node.name)}</h3><dl><div><dt>Goal metric</dt><dd>${esc(node.goal_metric)}</dd></div><div><dt>Current value</dt><dd>${esc(node.current_value)}</dd></div><div><dt>Delta</dt><dd>${esc(node.delta)}</dd></div></dl></article>`;
    }).join("");
    return `<section class="map-altitude"><header><h2>${esc(altitude.name)}</h2></header><div class="map-grid">${nodeHtml}</div></section>`;
  }).join("");
  return `<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>${esc(map.name)}</title><style>${styles}</style></head><body><header class="topbar"><div class="brand">VEDHA</div><div class="meta"><span>Business map</span></div></header><main><div class="map-title"><h1>${esc(map.name)}</h1></div>${altitudes}</main></body></html>`;
}
