...
function renderBusinessMap(map) {
  const altitudes = (map.altitudes ?? []).map((altitude) => {
    ...
    const nodeHtml = nodes.map((node) => {
      const steps = (node.steps ?? []).flatMap((step) => Object.values(step));
      return `<article class="map-node"><h3>${esc(node.name)}</h3><dl><div><dt>Goal metric</dt><dd>${esc(node.goal_metric)}</dd></div>${node.owner ? `<div><dt>Owner</dt><dd>${esc(node.owner)}</dd></div>` : ""}${node.cadence ? `<div><dt>Cadence</dt><dd>${esc(node.cadence)}</dd></div>` : ""}${node.under_workflow ? `<div><dt>Workflow</dt><dd>${esc(node.under_workflow)}</dd></div>` : ""}</dl>${steps.length ? `<ol class="activity-list">${steps.map((step) => `<li>${esc(step)}</li>`).join("")}</ol>` : ""}</article>`;
    }).join("");
    return `<section class="map-altitude"><header><div><p>Altitude</p><h2>${esc(altitude.name)}</h2></div><div class="altitude-meta"><span>${esc(altitude.type)}</span><strong>${esc(altitude.owner)}</strong></div></header><div class="map-grid">${nodeHtml}</div></section>`;
  }).join("");
  return `<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>${esc(map.name)}</title><style>${styles}</style></head><body><header class="topbar"><div class="brand">VEDHA</div><div class="meta"><span>Business map</span></div></header><main><div class="map-title"><p class="eyebrow">Atlas</p><h1>${esc(map.name)}</h1></div>${altitudes}</main></body></html>`;
}
...
.map-title{padding:12px 0 30px;border-bottom:1px solid #9da5a1}.map-title h1{font-size:38px;line-height:1.1;letter-spacing:0;margin:0}.map-altitude{padding:28px 0 34px;border-bottom:1px solid var(--line)}.map-altitude>header{display:flex;justify-content:space-between;gap:24px;align-items:end;margin-bottom:18px}.map-altitude>header p{margin:0 0 4px;color:#147357;font:700 10px ui-monospace,SFMono-Regular,monospace;text-transform:uppercase}.map-altitude>header h2{margin:0;font-size:24px}.altitude-meta{display:flex;gap:14px;align-items:center;font-size:12px;color:var(--muted)}.altitude-meta strong{color:var(--ink)}.map-grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(220px,1fr));gap:12px}.map-node{background:var(--paper);border:1px solid var(--line);border-radius:6px;padding:16px}.map-node h3{font-size:16px;margin:0 0 14px}.map-node dl,.semantic-model dl{margin:0}.map-node dl div,.semantic-model dl div{display:grid;grid-template-columns:92px minmax(0,1fr);gap:10px;padding:7px 0;border-top:1px solid #e5e8e6}.map-node dt,.semantic-model dt{color:var(--muted);font-size:11px}.map-node dd,.semantic-model dd{margin:0;font-size:12px;font-weight:650}.activity-list{margin:12px 0 0;padding-left:22px;color:#4f5a55;font-size:11px;line-height:1.7}.semantic-list{display:grid;grid-template-columns:repeat(auto-fit,minmax(300px,1fr));gap:14px;margin-top:24px}.semantic-model{background:var(--paper);border:1px solid var(--line);border-radius:6px;padding:18px}.semantic-model h2{font-size:17px;margin:0 0 14px}
