...
function atomBadge(atomId) {
  return `<span class="atom-badge">${esc(atomId)}</span>`;
}
...
function renderTarget(valueAtom, targetAtom) {
  ...
  return `<div class="target-view" data-atom="A1+A2">
    ...
    <div class="atom-pair">${atomBadge("A1")} ${atomBadge("A2")}</div>
  </div>`;
}

function renderValue(atom) {
  return `<div class="metric-view"><div><div class="metric-label">${esc(atom.title)}</div><div class="metric-value small">${esc(atom.display ?? atom.value)}</div></div>${atomBadge(atom.atom_id)}</div>`;
}
...
function renderComparison(atom) {
  ...
  return `<div class="comparison-view"><div class="visual-head"><strong>${esc(atom.title)}</strong>${atomBadge(atom.atom_id)}</div>${rows}<p class="atom-sentence">${esc(atom.sentence)}</p></div>`;
}
...
function renderStructural(atom) {
  ...
  return `<div class="structural-view"><div class="identity"><strong>${esc(atom.parent_label)}</strong><span>${esc(formatNumber(atom.parent, atom.unit))}</span><b>=</b><em>${esc(Object.keys(atom.children ?? {}).join(` ${atom.operator} `))}</em></div>${rows}<div class="visual-head foot">${atomBadge(atom.atom_id)}<span>${esc(atom.sentence)}</span></div></div>`;
}
...
function renderWaterfall(atom) {
  ...
  return `<div class="waterfall-view"><div class="visual-head"><strong>${esc(atom.title)}</strong>${atomBadge(atom.atom_id)}</div><div class="chart-scroll"><svg role="img" aria-label="${esc(atom.title)}" viewBox="0 0 ${width} ${height}"><line x1="28" y1="${plotBottom}" x2="872" y2="${plotBottom}" stroke="#d8ddda"/>${parts.join("")}</svg></div><p class="atom-sentence">${esc(atom.sentence)}</p></div>`;
}
...
function renderReview(review) {
  ...
  const sections = review.sections.map((section) => `<section class="review-section section-${esc(section.section)}"><div class="section-index">0${section.step}</div><div class="section-body"><div class="section-kicker">${esc(section.section.replaceAll("_", " "))}</div><h2>${esc(section.heading)}</h2><p class="question">${esc(section.question)}</p><p class="answer">${esc(section.answer)}</p>${renderInsights(section.insights)}${section.callout ? `<aside class="callout callout-${esc(section.callout.tone)}"><span>${esc(section.callout.label)}</span><strong>${esc(section.callout.text)}</strong></aside>` : ""}</div></section>`).join("");
...
const html = result.output_type === "review-object" ? renderReview(result) : renderLegacy(result);
